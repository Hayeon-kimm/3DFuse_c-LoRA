...
def perform_tuning(
    ...
    loss_sum = 0.0
    for current_dataloader in dataloaders: 
        for epoch in range(math.ceil(num_steps / len(dataloaders))):
...
def train(
    ...
    corgi_dataset = PivotalTuningDatasetCapation(
        instance_data_root='./results2/corgi/initial_image',
        token_map=token_map,
        use_template=use_template,
        tokenizer=tokenizer,
        size=resolution,
    ...
    brown_cat_dataset = PivotalTuningDatasetCapation(
        instance_data_root='./results2/brown_cat/initial_image',
        token_map=token_map,
        use_template=use_template,
        tokenizer=tokenizer,
        size=resolution,
    ...
    tiger_dataset = PivotalTuningDatasetCapation(
        instance_data_root='./results2/tiger/initial_image',
        token_map=token_map,
        use_template=use_template,
        tokenizer=tokenizer,
        size=resolution,
    ...
    if train_inpainting:
        ...
        corgi_dataloader = text2img_dataloader(
            corgi_dataset,
            train_batch_size,
            tokenizer,
            vae,
        ...
        brown_cat_dataloader = text2img_dataloader(
            brown_cat_dataset,
            train_batch_size,
            tokenizer,
            vae,
        ...
        tiger_dataloader = text2img_dataloader(
            tiger_dataset,
            train_batch_size,
            tokenizer,
            vae,
    ...
    unet.requires_grad_(False) #unet parameter update deactivation
    vae.requires_grad_(False)
    ...
    if perform_inversion:
        ...
        train_inversion( #brown_cat
            unet,
            vae,
            text_encoder,
            brown_cat_dataloader,
            max_train_steps_ti,
            cached_latents=cached_latents,
            accum_iter=gradient_accumulation_steps,
            scheduler=noise_scheduler,
            ...
            placeholder_tokens=placeholder_tokens,
            placeholder_token_ids=placeholder_token_ids,
            save_path='./results2/brown_cat/lora',
            test_image_path=instance_data_dir,
            log_wandb=log_wandb,
            wandb_log_prompt_cnt=wandb_log_prompt_cnt,
            class_token=class_token,
        ...
        # 주어진 이미지와 가장 일치하는 텍스트 표현을 찾는 역할

        del ti_optimizer

    # Next perform Tuning with LoRA:
    if not use_extended_lora: # HERE
        unet_lora_params, _ = inject_trainable_lora(
            unet,
            r=lora_rank,
            target_replace_module=lora_unet_target_modules,
    ...
    print("PTI : Before training:")
    inspect_lora(unet) #(굳이 return도 없는데 근데 뭣하러 하는거지..?)

    params_to_optimize = [
        {"params": itertools.chain(*unet_lora_params), "lr": unet_lr},
    ]

    text_encoder.requires_grad_(False) # text_encoder 학습 안하도록!

    if continue_inversion: # continue_inversion = True로 설정되어있음.
        params_to_optimize += [
            {
                "params": text_encoder.get_input_embeddings().parameters(),
                "lr": continue_inversion_lr
        ...
        text_encoder.requires_grad_(True) # text_encoder도 학습
        params_to_freeze = itertools.chain(
            text_encoder.text_model.encoder.parameters(),
            text_encoder.text_model.final_layer_norm.parameters(),
            text_encoder.text_model.embeddings.position_embedding.parameters(),
        ...
        text_encoder.requires_grad_(False)
    if train_text_encoder: # text_encoder도 같이 학습 예정
        text_encoder_lora_params, _ = inject_trainable_lora(
            text_encoder,
            target_replace_module=lora_clip_target_modules,
            r=lora_rank,
        ) # 이건 text_encoder의 LoRA
        params_to_optimize += [
            {
                "params": itertools.chain(*text_encoder_lora_params),
                "lr": text_encoder_lr,
    ...
    dataloaders = [train_dataloader, corgi_dataloader, brown_cat_dataloader, tiger_dataloader]
    #dataloaders = [train_dataloader, panda_dataloader]
    perform_tuning(
        unet,
        vae,
        text_encoder,
