...
import sys
device_glb = torch.device("cuda:0")
...
class SJC_3DFuse(BaseConf):
    ...
    def run(self):
        ...
        #save_dir=cfgs['sd'].pop('dir').split('/')[2]
        exp_instance_dir=os.path.join(cfgs.pop('exp_dir'),cfgs['save_name']) #exp_dir, initial에서 수정
        ...
        # # Optimization  and pivotal tuning for LoRA(2D)
        ...
        # Get coarse point cloud from off-the-shelf model(initial_image0으로 생성)
    ...
    with tqdm(total=n_steps) as pbar, \
        ...
        for i in range(len(poses_)):
            ...
            with torch.no_grad():
                ...
                zs = y + chosen_σs * noise # color + ? * noise
            ...
            if every(pbar, percent=2):
                with torch.no_grad():
                    y = model.decode(y)
                    try : 
                        vis_routine(metric, y, depth,p,depth_map[0]) #HERE
                    except : 
                        print("ERROR IS OCCURED IN {0}".format(i))
                        pass
